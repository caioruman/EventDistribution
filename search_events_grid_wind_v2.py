...
#        continue
      ...
      if os.path.exists(f"pr_wsn_{y}_{m:02d}_v4.p"):
        print("jump")
        continue
      ...
      for i in range(pr.shape[1]):
        for j in range(pr.shape[2]):
          ...
          #print('3')          
          events[i,j] = getEvents(pr[:,i,j], wsn[:,i,j], events[i,j], datei)
          #sys.exit()                      
...
class Event:
  def __init__(self, length, intensity, initialDate, wetSnow_count, wetSnow_total): #, total):
    self.length = length
